...
#    read_from_disk(parent_path)
    ...
    prevdir = ""
    if found.has_next():
        nextdir = pagedata[found.next_page_number()-1].Name#found.next_page_number()

    if found.has_previous():
        #prev = found.previous_page_number()
        prevdir = pagedata[found.previous_page_number()-1].Name#found.next_page_number()
...
def thumbnails(request, T_Url_Name=None):
    ...
    def make_thumbnail(thumb_file, thumb_size, mode="", FQPNDirectory=None):
        """
...
#        Thumbnails.objects.filter(
    ...
    def process_dir(fs_path, thumb_size, mode=""):
        ...
        if len(Thumbdata.SmallThumb) == 0 and files is not None:
            temp = return_image_obj(configdata["locations"]["albums_path"]+os.path.join(files.FQPNDirectory, files.Name))
            Thumbdata.SmallThumb = create_tnail_img(temp,
                                                    sizes["small"],
                                                    fext = fext)

        if files is None:
            temp = return_image_obj(configdata["locations"]["images_path"]+os.sep + configdata["filetypes"]["dir"][1])
            Thumbdata.SmallThumb = create_tnail_img(temp,
                                                    sizes["small"],
...
#        return imageicon("folder-close-icon.png", thumb_size)
            ...
            if entry[0].lower() == arch_filename:
                ...
                if file_data is not None:
                    ...
                    if fileext.lower() in translate.keys():
                        ...
                        #
                        thumbnailfile = imageicon(os.path.join(configdata["locations"]["resources_path"],
                                                               "images",
    ...
    def imageicon(icon_file, thumb_size):
        ...
        thumbnailfile = get_thumbnail(iconfile,
                                      "%sx%s" % (px_size, px_size),#(thumb_size, thumb_size),
                                      format="%s" % fext.lower(),
                                      crop=None,
                                      force=False)
        return thumbnailfile
...
#            source_image = source_image.convert('RGB')
    ...
    if "dir" in request.GET:
        return process_dir(fs_path, thumb_size)
...
#            fs_path = fs_path[:-1]
        ...
        sourcepath = request.path.lower().replace(os.sep, r"/").replace(r"/thumbnails/",
                                                                     r"/albums/")
        sourcepath, thumb_fn = os.path.split(sourcepath)
        thumb_fn = os.path.basename(request.path.strip())
        Thumbdata = Thumbnails_Files.objects.filter(FilePath=os.path.split(fs_path)[0],
...
def read_from_disk(dir_to_scan):
    ...
    def update_entry(entry, webpath):
        ...
        try:
            ...
            temp = recovery_from_multiple(entry, entry_fqfn)
        ...
        if temp.is_image != test_extension(entry.name,
                                            configdata["filetypes"]["graphic_file_types"]):
            temp.is_image = test_extension(entry.name,
                                            configdata["filetypes"]["graphic_file_types"])
            changed = True
