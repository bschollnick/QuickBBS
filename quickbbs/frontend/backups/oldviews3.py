...
#    read_from_disk(parent_path)
    ...
    if sorder == 0:
        pagedata = IndexData.objects.defer("LastScan", "LastMod", "Size", "NumFiles", "NumDirs", "ParentDirID").filter(FQPNDirectory=url_parent,
                                            is_dir=True,
                                            Ignore=False,
                                            DeletePending=False).order_by("-is_dir",
                                                                   "SortName")
    elif sorder == 1:
        pagedata = IndexData.objects.defer("LastScan", "LastMod", "Size", "NumFiles", "NumDirs", "ParentDirID").filter(FQPNDirectory=url_parent,
                                            is_dir=True,
...
def thumbnails(request, t_url_name=None):
    ...
    def process_dir(fs_path):
        ...
        try:
            ...
            files = IndexData.objects.values("Size", "FQPNDirectory", "Name", "is_dir", "Ignore", "is_image").filter(FQPNDirectory=webpath,
                                             is_dir=False,
                                             Ignore=False,
                                             DeletePending=False,
                                             is_image=True,
...
#            os.sep + "images" + os.sep + "1431973824_compressed.png"
        ...
        page = int(get_option_value(request, "arch", 0))
        if page == "":
            page = 0
...
def validate_database(dir_to_scan):
    ...
    temp = IndexData.objects.defer("SortName", "LastScan", "LastMod", "Size").filter(FQPNDirectory=webpath, Ignore=False)
    for entry in temp:
...
def get_db_files(sort_order, fpath):
    if sort_order == 0:
...
def archive_item(request):
    ...
    context["large"] = get_option_value(
        request, "size", configdata["configuration"]["large"])
    paths["archive_item"] = int(get_option_value(request, "a_item", 1)) - 1
    paths["item_fs"] = configdata["locations"]["albums_path"]\
