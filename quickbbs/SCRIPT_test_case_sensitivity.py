...
def test_case_insensitive_matching():
    ...
    for lower_name in sorted(matching_lower_names):
        ...
        db_name = next(n for n in db_file_names if n.lower() == lower_name)
...
def check_for_duplicate_lowercases():
    ...
    for directory in dirs:
        ...
        files = FileIndex.objects.filter(home_directory=directory, delete_pending=False).values_list("name", flat=True)
