...
class VideoBackend(AbstractBackend):
    ...
    def process_from_file(
        ...
        pillow_output = converter._process_pil_image(thumbnail, sizes, output_format, quality)
...
def _generate_thumbnail_to_pil(video_path, time_offset="00:00:10", width=320, height=240):
    """
...
def _generate_multiple_thumbnails_pil(video_path, count=5, width=320, height=240):
    ...
    for i in range(count):
        ...
        time_str = f"{int(time_offset_seconds // 3600):02d}:{int((time_offset_seconds % 3600) // 60):02d}:{int(time_offset_seconds % 60):02d}"
...
if __name__ == "__main__":
    ...
    output = processor.process_from_file(video_file, sizes={"small": (320, 240), "medium": (640, 480), "large": (1280, 720)}, output_format="PNG", quality=85)
    print(output.keys())
