"""Web-layer tests for thumbnails/views.py — thumbnail2_file / thumbnail2_dir."""
...
class TestThumbnailEdgeCases(ViewSmokeTestBase):
    """Edge-case branches of thumbnail2_file / thumbnail2_dir not covered by
    ...
    def test_file_thumbnail_invalid_size_normalizes_to_small(self):
        ...
        response = self.get(f"/thumbnail2_file/{self.file_obj.file_sha256}?size=huge")
    ...
    def test_directory_thumbnail_generic_icon_when_no_cover_image(self):
        ...
        response = self.get(f"/thumbnail2_directory/{empty_dir.dir_fqpn_sha256}")
        assert response.status_code == 200
