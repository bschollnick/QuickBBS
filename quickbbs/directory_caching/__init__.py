...
#            print (s_entry.is_dir())
            if s_entry.is_dir():
                #DirData = quickbbs.models.DirData()
                data.filename = ""
...
#                print ("LastMod - ", data.human_st_mtime)
#                print ("DirFQPN - ", data.fq_filename)
#                print ("NumFiles - ", data.number_files)
#                print ("NumDirs - ", data.number_dirs)
#                 if quickbbs.models.DirData.objects.all().filter(DirFQPN=data.fq_filename).exists():
#                     print("Already Exists in Database, may need refresh")
#                 else:
#                     quickbbs.models.DirData.objects.create(LastMod = datetime.datetime.fromtimestamp(data.st[stat.ST_MTIME]).strftime("%Y-%m-%d %H:%M:%S"),#data.st[stat.ST_MTIME],
#                                                            LastScan = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
#                                                            DirFQPN = data.fq_filename,
#                                                            DirURL = "",
...
#                    print("Already Exists in Database, may need refresh")
#                else:
#                    quickbbs.models.DirData.objects.create(LastMod = time.time())#data.st[stat.ST_MTIME],
#                                                           #LastScan = time.time(),
#                                                           DirFQPN = data.fq_filename,
#                                                           DirURL = "")#,
#                                                           NumFiles = data.number_files,
...
#                                                           Ignore = False)
#
#                     DirData.LastMod = data.st[stat.ST_MTIME]
#                     DirData.LastScan = time.time()
#                     DirData.DirFQPN = data.fq_filename
#                     DirData.DirURL = ""
#                     DirData.NumFiles = data.number_files
#                     DirData.NumDirs = data.number_dirs
#                     DirData.ParentDirID = -1
#                     DirData.ThumbFQFN = ""
#                     DirData.Ignore = False
#                    DirData.save()
#                data.archive_listings = None
            else:
#                 if quickbbs.models.FileData.objects.all().filter(FileName=s_entry.name).filter(FQPNDirectory=scan_directory).exists():
#                     print("Already Exists in Database, may need refresh")
#                 else:
#                     FileData = quickbbs.models.FileData()
#                     FileData.LastMod = s_entry.stat.ST_MTIME
#                     FileData.FileName = s_entry.name
#                     FileData.FileSize = s_entry.stat().ST_SIZE
#                     FileData.FQPNDirectory = scan_directory
#                     FileData.ParentDirID = -1
#                     FileData.Ignore = False
#                     FileData.DeletePending = False
#                     FileData.save()
                ...
                data.is_pdf = (data.file_extension == "pdf")
...
#####################################################
    ...
    def _return_filtered_dir_count(self, scan_directory):
        ...
        try:
            ...
            print ("StopIteration received on dir scan")
