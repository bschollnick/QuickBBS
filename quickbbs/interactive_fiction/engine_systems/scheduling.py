...
time-driven behavior on top of — never an ASFA-specific implementation
(see the plan's "generic framework, not an ASFA-specific implementation"
design section). This module has zero knowledge of what a "character," a
...
resolution. A story with a coarser or finer native clock (e.g. ASFA's own
`nTime`, 5 minutes per tick) converts to/from minutes at its own
boundary — see `asfa_scheduling.py` for exactly that conversion — this
...
# jump amounts (e.g. ASFA's own real cheat-menu constants — see
# asfa_scheduling.py) defines those itself rather than using these.
...
# 2026-08-22) — no config schema of its own yet (SchedulingState isn't
# sourced from a StorySystemConfig row today; a story's own clock value is
# threaded through explicitly, per this module's own docstring).
