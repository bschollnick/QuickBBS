...
class TestFsCacheTrackingModel:
    ...
    def test_model_creation_with_values(self):
        ...
        cache_entry = fs_Cache_Tracking.objects.create(directory_sha256=test_sha, DirName=test_dirname, lastscan=test_timestamp, invalidated=True)
    ...
    def test_model_indexes_exist(self):
        ...
        composite_index_exists = any(set(idx.fields) == {"directory_sha256", "invalidated"} for idx in meta.indexes)
...
class TestFsCacheTrackingStaticMethods:
    ...
    def test_clear_all_records_with_data(self):
        ...
        for i in range(3):
            fs_Cache_Tracking.objects.create(directory_sha256=f"{'a' * 63}{i}", DirName=f"/test/dir_{i}", invalidated=False)
...
class TestFsCacheTrackingInstanceMethods:
    ...
    def test_add_to_cache_update_existing(self, mock_time, mock_get_dir_sha):
        ...
        existing_entry = fs_Cache_Tracking.objects.create(
            directory_sha256=self.test_sha, DirName=self.test_dirname, lastscan=1000000000.0, invalidated=True
        )
    ...
    def test_sha_exists_in_cache_true(self):
        ...
        fs_Cache_Tracking.objects.create(directory_sha256=self.test_sha, invalidated=False)
    ...
    def test_sha_exists_in_cache_false_invalidated(self):
        ...
        fs_Cache_Tracking.objects.create(directory_sha256=self.test_sha, invalidated=True)
    ...
    def test_remove_from_cache_sha_success(self, mock_index_dirs, mock_time):
        ...
        fs_Cache_Tracking.objects.create(directory_sha256=self.test_sha, invalidated=False, lastscan=1000000000.0)
    ...
    def test_clear_layout_cache(self, mock_hashkey, mock_layout_manager):
        ...
        layout_manager_cache.update({"key_page_1": "data1", "key_page_2": "data2", "key_page_3": "data3", "other_key": "other_data"})
...
class TestFsCacheTrackingQueries:
    ...
    def test_index_usage_composite_query(self):
        ...
        # Create test data
        for i in range(10):
            fs_Cache_Tracking.objects.create(directory_sha256=f"{'a' * 63}{i}", invalidated=i % 2 == 0)  # Mix of True/False

        # Query that should use the composite index
        results = fs_Cache_Tracking.objects.filter(directory_sha256="a" * 63 + "5", invalidated=False)

        assert results.count() == 0  # This specific combo doesn't exist

        # Query for existing combo
        results = fs_Cache_Tracking.objects.filter(directory_sha256="a" * 63 + "4", invalidated=True)
    ...
    def test_bulk_operations_performance(self):
        ...
        entries = []
        for i in range(100):
            entries.append(fs_Cache_Tracking(directory_sha256=f"{'a' * 63}{i:01d}", DirName=f"/test/dir_{i}", invalidated=False))
        ...
        with transaction.atomic():
            updated_count = fs_Cache_Tracking.objects.filter(directory_sha256__in=sha_list).update(invalidated=True)
