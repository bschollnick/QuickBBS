...
def return_plugin(sourcefile):
    ...
    If the plugin is not registered, return False
    """
...
def load_plugins():
    ...
    plugin_manager.setPluginPlaces([os.path.expanduser(os.path.abspath('.' + os.sep + "plugins"))])
    ...
    for x in sorted(FILE_TYPES_MAP):
        print "Loading FileType Plugin for %5s - %s" % (x, FILE_TYPES_MAP[x][2])
